...

  {
    ...
    while(std::getline(in, line)){
      ...
      if(key=="prompt.cwd"){
        ...
        if(lowered == "default"){
          g_settings.promptInputEllipsisRightWidth = -1;
        }else{
          try{
            int v = std::stoi(val);
...
inline void save_settings(const std::string& path){
  ...
  std::string rightWidthValue = (g_settings.promptInputEllipsisRightWidth < 0)
                                ? std::string("default")
                                : std::to_string(g_settings.promptInputEllipsisRightWidth);
  out << "prompt.input_ellipsis.right_width=" << rightWidthValue << "\n";
