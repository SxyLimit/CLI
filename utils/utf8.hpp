...
inline size_t utf8CharLength(unsigned char lead){
...
inline bool isInRange(char32_t cp, const CodepointRange& range){
  return cp >= range.first && cp <= range.last;
}

inline bool isCombining(char32_t cp){
...
inline int codepointWidth(char32_t cp){
  if(cp == 0) return 0;
...
inline std::vector<Utf8Glyph> utf8Glyphs(const std::string& text){
