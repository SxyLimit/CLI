...
static SplitWord timeSplitContext(const std::string& buf){
  if(todoWizardActive()){
    return {"", buf};
  }
  return splitLastWord(buf);
}

static std::string renderTimeTemplateLabel(const std::string& templ, const std::string& input){
  std::string label = templ;
  size_t inputIndex = 0;
  for(size_t i=0;i<label.size() && inputIndex<input.size(); ++i){
    char t = templ[i];
    unsigned char ch = static_cast<unsigned char>(input[inputIndex]);
    if(isTimeTemplatePlaceholder(t)){
      if(std::isdigit(ch)){
        label[i] = static_cast<char>(ch);
        ++inputIndex;
      }else{
        break;
      }
    }else{
      if(static_cast<char>(ch) == t){
        ++inputIndex;
      }
    }
  }
...
static Candidates timeCandidates(const std::string& buf){
  Candidates cand;
  auto sw = timeSplitContext(buf);
  std::set<std::string> seen;
  auto addCandidate = [&](const std::string& insertion, const std::string& display, const std::vector<int>& positions){
    std::string full = sw.before + insertion;
    if(seen.insert(full).second){
      cand.items.push_back(full);
      cand.labels.push_back(display);
  ...
  for(const auto& templ : kTemplates){
    ...
    if(buildTimeTemplateMatch(sw.word, templ, match)){
      ...
      std::string label = renderTimeTemplateLabel(templ, sw.word);
      if(match.text.empty() && sw.word.empty()) label = templ;
      addCandidate(insertion, label, match.positions);
    }
  }
