...
- `setting set prompt.input_ellipsis.enabled <true|false>`：开启后，当输入或自动补全内容超过指定长度时，将只保留最后的阈值宽度并用前缀的 `.` 标记被截断的部分，避免光标被推到屏幕之外。
- `setting set prompt.input_ellipsis.max_width <列宽>`：配置触发省略的最大显示宽度（单位为等宽字符数），默认值为 `60`，必须大于等于 3。
